...
```dockerfile
FROM zeroweb/alpine-docker-nginx-php82
...
COPY html /var/www/html/
RUN composer install --optimize-autoloader --no-interaction --no-progress
RUN echo "session.save_handler = redis" >> /etc/php82/conf.d/custom.ini && \
    echo 'session.save_path = "tcp://redis:6379"' >> /etc/php82/conf.d/custom.ini
```
